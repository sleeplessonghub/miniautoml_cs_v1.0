      ...
      with st.form('target_variable_selection_form'):
        ...
        if target != '-':
          if train[target].dtypes == float or train[target].dtypes == int:
            ...
            st.dataframe(train[target].value_counts(sort = True).astype(str), column_config = {target: st.column_config.Column(width = "medium"), "count": st.column_config.Column(width = "medium")})
