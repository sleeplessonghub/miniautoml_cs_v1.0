...
if st.session_state['df_pp'] is not None:
  ...
  with st.form('data_type_specification_form', height = 360):
    st.write(tw.dedent(
        """
        ...
        if target != '-':
          if train[target].dtypes == float or train[target].dtypes == int:
            ...
            if train[target].nunique() <= 5:
              ...
              st.dataframe(train[target].value_counts(sort = True).rename('Category Frequency').reset_index().astype(str), height = 'auto', hide_index = True, column_config = {target: st.column_config.Column(width = 185), 'Category Frequency': st.column_config.Column(width = 200)})
            elif train[target].nunique() > 5:
              st.write('Class 1 Label Selection (Scrollable):')
              st.dataframe(train[target].value_counts(sort = True).rename('Category Frequency').reset_index().astype(str), height = 213, hide_index = True, column_config = {target: st.column_config.Column(width = 185), 'Category Frequency': st.column_config.Column(width = 200)})
