      ...
      with st.form('target_variable_selection_form'):
        ...
        if target != '-':
          if train[target].dtypes == float or train[target].dtypes == int:
            ...
            st.dataframe(train[target].value_counts(sort = True).astype(str), height = 250, column_config = {target: st.column_config.Column(width = 400), "count": st.column_config.Column(width = 200)})
